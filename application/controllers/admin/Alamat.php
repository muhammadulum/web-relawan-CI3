...
class Alamat extends CI_Controller {
	...
	public function index()
	{
		...
		if ($id_admin) {
			...
			$data["aksi"] = $this->Alamat_model->getAll();
    ...
    {
        $product = $this->Alamat_model;       
            $product->save();
			if($this->db->affected_rows()>0)
			{
				echo "<script>alert('data berhasil ditambahkan');</script>";
			}
			echo "<script>window.location='".site_url('admin/alamat')."';</script>";
    ...
    public function editalamat($id = null)
    {
